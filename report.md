...
## 11. 小组成员分工
- 刘诚宜：基础 GMM+MFCC 实验代码完成，实验报告撰写
- 张嘉烁：拓展增加 MFCC+Δ+ΔΔ 与 n_components 对比实验，对比结果分析与实验报告撰写

---

## 12. 提交内容说明
- 代码：完整 src/ 目录与运行脚本（experiment.py / eval.py）
- 报告：本实验报告（含实验流程、原理解释、结果表格、可视化与错误分析）
- 实验产物：`artifacts/experiments_gmm_k/`（模型、eval.json、results.csv、plots 等）